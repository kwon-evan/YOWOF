...
from yowof.utils.vis_tools import convert_tensor_to_cv2img, vis_detection
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="clip",
        type=str,
        choices=["clip", "stream", "semi_stream"],
        help="inference mode",
    )
...
@torch.no_grad()
def inference_ucf_jhmdb_stream(
    args, d_cfg, model, device, dataset, class_names=None, class_colors=None
):
    # path to save
    if args.save:
        save_path = os.path.join(
            args.save_folder, args.dataset, args.version, "video_stream"
        )
    ...
    for index in range(args.start_index, len(dataset)):
        ...
        key_frame = convert_tensor_to_cv2img(
            key_frame_tensor, d_cfg["pixel_mean"], d_cfg["pixel_std"]
        )
...
@torch.no_grad()
def inference_ucf_jhmdb_semi_stream(
    args, d_cfg, model, device, dataset, class_names=None, class_colors=None
):
    # path to save
    if args.save:
