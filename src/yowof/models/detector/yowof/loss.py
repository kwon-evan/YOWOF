...
from yowof.utils.box_ops import box_cxcywh_to_xyxy, box_iou, generalized_box_iou
from yowof.utils.distributed_utils import get_world_size, is_dist_avail_and_initialized
from yowof.utils.misc import AVA_FocalLoss, Sigmoid_FocalLoss
from yowof.utils.vis_tools import vis_targets
...
class Criterion(object):
    def __init__(
        ...
        # loss
        if multi_hot:
            self.cls_loss = AVA_FocalLoss(
                device=device, gamma=0.5, num_classes=num_classes, reduction="none"
            )
            # self.cls_loss = Sigmoid_FocalLoss(alpha=alpha, gamma=gamma, reduction='none')
        else:
            self.cls_loss = Sigmoid_FocalLoss(
                alpha=alpha, gamma=gamma, reduction="none"
            )

    def prepare_targets(
        self, cls_pred_shape, targets, indices, src_idx, ignore_idx, pos_ignore_idx
    ):
        if self.multi_hot:
            # gt_cls: 0 -> bg; 1 -> fg; -1 -> ignore;
            # [BM, C]
            gt_cls = torch.zeros(cls_pred_shape, dtype=torch.int64, device=self.device)
            gt_cls[ignore_idx, :] = -1
            # [BN, C]
            tgt_cls_o = torch.cat(
    ...
    def loss_bboxes(self, pred_box, tgt_boxes, num_boxes):
        ...
        # giou loss
        loss_bboxes = 1.0 - torch.diag(gious)
    ...
    def __call__(self, outputs, targets):
        ...
        indices = self.matcher(
            pred_boxes=box_pred, anchor_boxes=anchor_boxes, targets=targets
        )
        ...
        for batch_index in range(batch_size):
            ...
            iou, _ = box_iou(
                box_pred[batch_index, ...], targets[batch_index]["boxes"].clone()
            )
            if iou.numel() == 0:
            ...
            a_iou, _ = box_iou(
                anchor_boxes[batch_index], targets[batch_index]["boxes"].clone()
            )
            if a_iou.numel() == 0:
        ...
        src_idx = torch.cat(
            [
                src + idx * anchor_boxes[0].shape[0]
        ...
        # prepare targets
        (gt_cls, foreground_idxs, num_foreground) = self.prepare_targets(
        ...
        loss_dict = dict(
            loss_labels=loss_labels, loss_bboxes=loss_bboxes, losses=losses
        )
