import numpy as np
import torch
from yowof.utils.box_ops import box_xyxy_to_cxcywh
...
class UniformMatcher(object):
    ...
    def __call__(self, pred_boxes, anchor_boxes, targets):
        ...
        cost_bbox_anchors = torch.cdist(anchor_boxes, box_xyxy_to_cxcywh(tgt_bbox), p=1)
        ...
        sizes = [
            len(v["boxes"]) for v in targets
        ]  # the number of object instances in each image
